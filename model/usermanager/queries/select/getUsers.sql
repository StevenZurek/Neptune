...
FROM `user`
    ...
    LEFT JOIN `user_address` AS user_profile_address ON `user_profile`.`currentAddress` = `user_profile_address`.`addressId`

    LEFT JOIN `user_billingprofile` ON `user`.`userId` = `user_billingprofile`.`userId`
    LEFT JOIN `user_address` AS user_billing_address ON `user_billingprofile`.`currentAddress` = `user_billing_address`.`addressId`

    LEFT JOIN `user_shippingprofile` ON `user`.`userId` = `user_shippingprofile`.`userId`
    LEFT JOIN `user_address` AS user_shipping_address ON `user_shippingprofile`.`currentAddress` = `user_shipping_address`.`addressId`
WHERE `user`.`userId` <> 0
